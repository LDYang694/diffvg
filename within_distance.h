...
DEVICE
inline
return_bool_with_value within_distance_with_z(const Path &path, const BVHNode *bvh_nodes, const Vector2f &pt, float r) {
    auto num_segments = path.num_base_points;
    constexpr auto max_bvh_size = 128;
    int bvh_stack[max_bvh_size];
    auto stack_size = 0;
    ...
    while (stack_size > 0) {
        ...
        if (node.child1 < 0) {
            ...
            //     }
            // } else 
            if (path.num_control_points[base_point_id] == 2) {
                // Cubic Bezier curve
                auto i0 = point_id;
                auto i1 = point_id + 1;
                auto i2 = point_id + 2;
                ...
                auto p2 = Vector2f{path.points[2 * i2], path.points[2 * i2 + 1]};
                auto p3 = Vector2f{path.points[2 * i3], path.points[2 * i3 + 1]};
                // auto zz = Vector4f{path.z_values[0], path.z_values[0], path.z_values[0], path.z_values[0]};
                auto zz = Vector4f{path.z_values[i0], path.z_values[i1], path.z_values[i2], path.z_values[i3]};
                // auto zz = Vector4f{0.1,0.2,0.3,0.4};
                auto eval = [&](float t) -> Vector2f {
                    auto tt = 1 - t;
                    return (tt*tt*tt)*p0 + (3*tt*tt*t)*p1 + (3*tt*t*t)*p2 + (t*t*t)*p3;
                };
                auto eval_z = [&](float t)->float {
                    auto tt = 1 - t;
                    return (tt*tt*tt)*zz[0] + (3*tt*tt*t)*zz[1] + (3*tt*t*t)*zz[2] + (t*t*t)*zz[3];
                };
                ...
                if (distance_squared(eval(0), pt) < r0*r0) {
                    // return 1.f;
                    // return {true, eval_z(0)};
                    return return_bool_with_value{true,eval_z(0)};
                }
                if (distance_squared(eval(1), pt) < r3*r3) {
                    // return 1.f;
                    // return {true, eval_z(1)};
                    return return_bool_with_value{true,eval_z(1)};
                }
                // The curve is (1 - t)^3 p0 + 3 * (1 - t)^2 t p1 + 3 * (1 - t) t^2 p2 + t^3 p3
                // = (-p0+3p1-3p2+p3) t^3 + (3p0-6p1+3p2) t^2 + (-3p0+3p1) t + p0
                // Want to solve (q - pt) dot q' = 0
                ...
                for (int j = 0; j < num_intervals + 1; j++) {
                    ...
                    if (distance_squared(eval(t), pt) < r * r) {
                        // return 1.f;
                        return return_bool_with_value{true,eval_z(t)};
                    }
                    if (upper_bound >= 1.f) {
                        break;
                    }
    ...
    // return -1.f;
    return return_bool_with_value{false,-1.f};
}
DEVICE
inline
...
DEVICE
inline
return_bool_with_value within_distance_with_z(const Shape &shape, const BVHNode *bvh_nodes, const Vector2f &pt, float r) {
    switch (shape.type) {
        // case ShapeType::Circle:
        //     return within_distance(*(const Circle *)shape.ptr, pt, r);
        // case ShapeType::Ellipse:
        ...
        //     return false;
        case ShapeType::Path:
            return within_distance_with_z(*(const Path *)shape.ptr, bvh_nodes, pt, r);
        // case ShapeType::Rect:
        //     return within_distance(*(const Rect *)shape.ptr, pt, r);
    }
    assert(false);
...
DEVICE
inline
return_bool_with_value within_distance_with_z(const SceneData &scene,
                     int shape_group_id,
                     const Vector2f &pt) {
    const ShapeGroup &shape_group = scene.shape_groups[shape_group_id];
    // pt is in canvas space, transform it to shape's local space
    ...
    while (stack_size > 0) {
        ...
        if (node.child1 < 0) {
            ...
            auto shape_id = node.child0;
            const auto &shape = scene.shapes[shape_id];
            return_bool_with_value bool_with_z = within_distance_with_z(shape, scene.path_bvhs[shape_id],
                                local_pt, shape.stroke_width);
            if (bool_with_z.return_bool) {
                // return 1.f;
                return return_bool_with_value{true,bool_with_z.value};
            }
        } else {
            assert(node.child0 >= 0 && node.child1 >= 0);
            const AABB &b0 = bvh_nodes[node.child0].box;
...
return_bool_with_value within_distance_with_z(const SceneData &scene,
    ...
    if (edge_query == nullptr || shape_group_id != edge_query->shape_group_id) {
        // Specialized version
        return within_distance_with_z(scene, shape_group_id, pt);
    }
    const ShapeGroup &shape_group = scene.shape_groups[shape_group_id];
    // pt is in canvas space, transform it to shape's local space
    auto local_pt = xform_pt(shape_group.canvas_to_shape, pt);
    ...
    // float ret = -1.f;
    return_bool_with_value ret{false,-1.f};
    while (stack_size > 0) {
        const BVHNode &node = bvh_nodes[bvh_stack[--stack_size]];
        if (node.child1 < 0) {
            // leaf
            auto shape_id = node.child0;
            const auto &shape = scene.shapes[shape_id];
            return_bool_with_value bool_with_value = within_distance_with_z(shape, scene.path_bvhs[shape_id],
                                local_pt, shape.stroke_width);
            if (bool_with_value.return_bool) {
                ret = bool_with_value;
                if (shape_id == edge_query->shape_id) {
                    edge_query->hit = true;
                }
